// ゲームタブ: ルーレット (0〜36 のシングルゼロ)
// 出目・配当は Cloud Function (casino の spin) が決める。
...
function totalBet() {
    let sum = 0;
    roulette.bets.forEach(bet => { sum += bet.amount; });
    return sum;
}
...
function placeBet(type, value) {
    ...
    const amount = Math.min(roulette.chip, remaining);
    if (amount < 1) {
        showMessage(el('casino-table-message'), '手元のチップを使い切っています。', 'info');
        return;
    }
    const key = betKey(type, value);
    const current = roulette.bets.get(key);
    roulette.bets.set(key, { type, value, amount: (current ? current.amount : 0) + amount });
    roulette.placed.push({ key, amount });
    renderBetControls();
}

function undoBet() {
    const last = roulette.placed.pop();
    if (!last) return;
    const bet = roulette.bets.get(last.key);
    if (!bet) return;
    bet.amount -= last.amount;
    if (bet.amount <= 0) roulette.bets.delete(last.key);
    renderBetControls();
}
...
function rebet() {
    if (!roulette.lastBets.length || !casino.session) return;
    const total = roulette.lastBets.reduce((sum, bet) => sum + bet.amount, 0);
    ...
    roulette.lastBets.forEach(bet => {
        ...
        roulette.bets.set(key, { ...bet });
        roulette.placed.push({ key, amount: bet.amount });
    });
    renderBetControls();
}
...
function renderBetControls() {
    document.querySelectorAll('#roulette-board [data-type]').forEach(cell => {
        ...
        const bet = roulette.bets.get(betKey(cell.dataset.type, value));
    ...
    document.querySelectorAll('.casino-chip-picker [data-chip]').forEach(button => {
        button.setAttribute('aria-checked', String(Number(button.dataset.chip) === roulette.chip));
    ...
    el('casino-undo-button').disabled = casino.busy || roulette.placed.length === 0;
    el('casino-clear-button').disabled = casino.busy || roulette.bets.size === 0;
    el('casino-rebet-button').disabled = casino.busy || roulette.lastBets.length === 0;
}
...
function spinWheelTo(number) {
    ...
    const current = roulette.wheelRotation;
    const target = -pocketAngle;
    const offset = ((target - current) % 360 + 360) % 360;
    roulette.wheelRotation = current + 360 * 5 + offset;
    wheel.style.transform = `rotate(${roulette.wheelRotation}deg)`;

    return delay(prefersReducedMotion() ? 0 : ROULETTE_SPIN_MS);
}

async function spin() {
    if (casino.busy || !casino.session) return;
    const bets = Array.from(roulette.bets.values());
    if (!bets.length) return;

    setCasinoBusy(true);
    el('casino-spin-button').setAttribute('aria-busy', 'true');
    ...
    try {
        ...
        roulette.lastBets = bets.map(item => ({ ...item }));
        roulette.bets.clear();
        roulette.placed = [];
        ...
        el('casino-spin-button').removeAttribute('aria-busy');
        setCasinoBusy(false);
    }
}
...
function loadChipPreference() {
    try {
        ...
        if ([1, 5, 10, 50, 100].includes(saved)) roulette.chip = saved;
    } catch (error) {
        // 保存できない環境では既定値のまま
    }
}
...
function initRoulette() {
    ...
    document.querySelectorAll('.casino-chip-picker [data-chip]').forEach(button => {
        button.addEventListener('click', () => {
            roulette.chip = Number(button.dataset.chip);
            try { localStorage.setItem(CASINO_CHIP_STORAGE_KEY, String(roulette.chip)); } catch (error) { /* 無視 */ }
            renderBetControls();
        });
    });
