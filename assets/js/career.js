...
function buildSpiQuestions() {
    ...
    for (let i = 0; i < 17; i++) {
        ...
        const answer = unitPrice * count - discount;
        addNumeric(
            'SPI文章題',
            `1個${unitPrice}円の商品を${count}個買い、合計から${discount}円引きされた。支払額はいくらですか。`,
            answer,
            '円',
            `単価×個数から値引き額を引きます。${unitPrice}×${count}-${discount}です。`,
            [unitPrice * count, answer + discount, answer - unitPrice, count * discount]
        );
    }
